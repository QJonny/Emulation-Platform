...
void set_up_routing_tables (uint chip, uint core)		// this is the tricky bit!
{
  ...
  int x = (core-1)&3;						// core x, y coordinates
  int y = (core-1)>>2;
  uint mc_table_entry = core;					// track MC table allocation (crudely!)
...
void add_route_to_core (int i, int j)				// filters out off-chip neighbours
{
  if (i>-1 && i<4  && j>-1 && j<4				// check for end of row & col		
     && !(i==x && j==y)) route += 1<<(i+4*j+7);			// don't route to self
}

uint next_chip (uint dir)					// find next chip, modulo array size
{
  uint cX = chip>>8; uint cY = chip&255;
  if (dir&SOUTH) cY = cY ? cY-1 : YMAX-1;			// do the modulo sums
  if (dir&NORTH) cY = (cY+1)%YMAX;
  if (dir&WEST)  cX = cX ? cX-1 : XMAX-1;
  if (dir&EAST)  cX = (cX+1)%XMAX;
  return (cX<<8)+cY;
}

void add_route_to_chip (uint dir)				// dir specifies possible directions
{
  if (TOROID) route += dir;
  else
  {
    uint nc = next_chip(dir);					// toroidal routing on...
    uint cX  = chip>>8; uint cY  = chip&255;			// ...non-toroidal system
    uint ncX = nc>>8;   uint ncY = nc&255;			// uses default routing to get back
    if (cX!=ncX) route += toH[ncX>cX];	
    if (cY!=ncY) route += toV[ncY>cY];		
  }
}

void set_mc_table_entry (uint chip)				// route from (chip, core)
{
  spin1_set_mc_table_entry(mc_table_entry, (chip<<8)+core, 0xffffffff, route);
  mc_table_entry += 16;						// = number of app cores/chip
...
// first set up routes for internally-generated packets
  ...
  {
    for (int j=y-1; j<y+2; j++)	add_route_to_core(i,j);		// row below, same, above
  }
  if (edge(x)) add_route_to_chip (toH[x&1]);			// send off-chip where appropriate
  if (edge(y)) add_route_to_chip (toV[y&1]);
  set_mc_table_entry(chip);					// write local Router table entry
...
// ...each core handles routes from corresponding cores on other chips
  ...
  {
    for (int i=x-1; i<x+2; i++) add_route_to_core(i,3-y);	// route to opposite row
    if (edge(x)) add_route_to_chip (toH[x&1]);			// forward diagonal connection
    set_mc_table_entry(next_chip(fromV[y&1]));
  }
	
  if (edge(x))							// left & right
  {
    for (int j=y-1; j<y+2; j++) add_route_to_core(3-x,j);	// route to opposite col
    set_mc_table_entry(next_chip(fromH[x&1]));
  }

  if (edge(x) && edge(y))					// incoming diagonal connection
  {
    add_route_to_core(3-x,3-y);					// receive from opposite corner
    set_mc_table_entry(next_chip(fromH[x&1]+fromV[y&1]));
  }
}
...
void c_main (void)
{
  ...
  {
    io_printf (IO_DBG, "Started core %d %d %d\n",
    ...
    spin1_callback_on (MC_PACKET_RECEIVED, packet_in, -1);	// incoming packet callback

    spin1_start();						// start event-driven operation

    io_printf (IO_DBG, "Terminated core %d %d %d\n",
		chip>>8, chip&255, core);			// print if event_stop used...
  }
}

...
*  Last modified on : $Date: 2012-11-17 11:32:43 +0000 (Sat, 17 Nov 2012) $
*  Last modified by : $Author: plana $
*  $Id: heat_demo.c 2074 2012-11-17 11:32:43Z plana $
...
#define IS_NORTHERNMOST_CHIP(chip) ((chip_map[chip] & (1 << NORTH)) != 0)
#define IS_SOUTHERNMOST_CHIP(chip) ((chip_map[chip] & (1 << SOUTH)) != 0)
#define IS_EASTERNMOST_CHIP(chip)  ((chip_map[chip] & (1 << EAST)) != 0)
#define IS_WESTERNMOST_CHIP(chip)  ((chip_map[chip] & (1 << WEST)) != 0)
...
/* report results in shared memory (SDRAM) */
static volatile int * const core_temp = (int *) (SPINN_SDRAM_BASE + 0x1000
                                                   + 16 * sizeof(uint));
...
*
* SYNOPSIS
*  void routing_table_init (uint bside)
*
* SOURCE
*/
void routing_table_init (uint bside)
{
  ...
  {
    if (IS_NORTHERNMOST_CHIP(my_chip))
    {
  ...
  {
    if (IS_SOUTHERNMOST_CHIP(my_chip))
    {
  ...
  {
    if (IS_EASTERNMOST_CHIP(my_chip))
    {
  ...
  {
    if (IS_WESTERNMOST_CHIP(my_chip))
    {
  ...
  if (route_from_west)
  {
    spin1_set_mc_table_entry((6 * coreID) + 4,     // entry
                             west_key,             // key
                             0xffffffff,           // mask
                             ROUTE_TO_CORE(coreID) // route
                            );
  ...
  /* ------------------------------------------------------------------- */
  /* avoid duplication -- only core 1 does this                          */
  if (coreID == 1)
  {
    // setup routing entries for synchronization barrier
    // setup the local (on-chip) routes
    uint loc_route = core_map[my_chip] & 0xfffffffe;
    ...
    spin1_set_mc_table_entry(200,                           // entry
                             STOP_KEY,                      // key
                             CMD_MASK,                      // mask
    ...
    if (IS_NORTHERNMOST_CHIP(my_chip))
    {
      ...
      spin1_set_mc_table_entry(201,                         // entry
                               TEMP_NORTH_KEY,              // key
                               0xffffffff,                  // mask
    ...
    if (IS_EASTERNMOST_CHIP(my_chip))
    {
      ...
      spin1_set_mc_table_entry(202,                         // entry
                               TEMP_EAST_KEY,               // key
                               0xffffffff,                  // mask
    ...
    if (IS_SOUTHERNMOST_CHIP(my_chip))
    {
      ...
      spin1_set_mc_table_entry(203,                         // entry
                               TEMP_SOUTH_KEY,              // key
                               0xffffffff,                  // mask
    ...
    if (IS_WESTERNMOST_CHIP(my_chip))
    {
      ...
      spin1_set_mc_table_entry(204,                         // entry
                               TEMP_WEST_KEY,               // key
                               0xffffffff,                  // mask
...
void sdp_init ()
{
  ...
  my_msg.dest_addr = 0;   		// Root chip - CP SYSRAM is not initialised
  // fill in SDP source & flag fields
  my_msg.flags = 0x07;
  my_msg.srce_port = coreID;
  //my_msg.srce_addr = sv->p2p_addr;
  my_msg.srce_addr = spin1_get_chip_id();  // This chip ID - CP SYSRAM is not initialised with this value
}
/*
*******/
...
void report_results ()
{
  ...
  spin1_delay_us(2000000);		// CP for spin_emu
  io_printf (IO_STD, "T = %7.3f\n", my_temp);
}
/*
*******/
...
/****f* heat_demo.c/get_core_map_size
*
* SUMMARY
*
* SYNOPSIS
*  void get_core_map_size ()
*
* OUTPUTS
*  uint = core_map size
*
* SOURCE
*/
uint get_core_map_size (uint chips)
{
  uint i;

  // TODO: Works square core maps only!
  uint size = 0;
  for (i = 0; i < 32; i++)
  {
...
void c_main()
{
  ...
  uint bsize = get_core_map_size (NUMBER_OF_CHIPS);
  uint bside = 1 << bsize;
  my_x = chipID >> 8;
  my_y = chipID & 0xff;
  ...
  if ((core_map[my_chip] & (1 << coreID)) != 0)
  {
    // TODO: Works square core maps only!
    ...
    // core <0, 0, 1> takes care of host communication
    if ((chipID == CHIP_ADDR(0, 0)) && (coreID == 1))
    {
      spin1_callback_on (SDP_PACKET_RX, host_data, 0);
    }

    // initialise routing tables
    routing_table_init (bside);
